...
const ListViewFeed = () => {
	...
	const handleDelete = async () => {
		if (todos_home.length !== 0) {
			const has = todos_home.filter((todo) => todo.category === list_info.name);
		...
		await listDB.lists
			.filter(
				(list) => list.list_id === list_info.list_id && list.default === false
			)
			.delete();
		await db.todos
						...
						<Done>
							<div className="done_options">
								...
								<div className="done_text">
									We could not find tasks from the list "{list_info.name}"
								</div>
							</div>
						</Done>
					) : (
