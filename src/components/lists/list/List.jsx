		...
		if (text) {
			if (!props.default) {
				...
				const completed_count = props.users_lists.filter(
					(todo) => todo.category === props.name && todo.complete === 1
				).length;
	...
	return (
		<div style={{ margin: "0 auto" }}>
				...
				<div className="list">
					<div className="list-content">
						{edit_text ? (
							...
							<>
								{props.name ? (
									<Link to={`/list_${props.id}`} onClick={handleDispatch}>
										<div
											className="list-name"
											style={{
												color: darkMode ? "white" : "black",
										...
										<div className="list-name" style={{ color: "grey" }}>
											{todo_count} To do's
										</div>
										{todo_count - completed_count === 0 ? null : (
											<div className="list-todos">
												{todo_count - completed_count} Not done
											</div>
										)}{" "}
									</Link>
								) : (
			...
			{props.default ? null : (
				<>
					{edit_text ? (
						<span className="list-end" onClick={handleEditFinish}>
							<img
								src={
									!text
										? darkMode
