    ...
    const GET = {
        ...
        lists: () => {
            ...
            const fetch_important_list = async () => {
                ...
                const completed = await todoDB.todos
                    .filter(
                        (todo) => todo.complete === 1 && todo.important === 1
                    )
                    .count();

                dispatch(
                    dispatch_list_important({ todo: important, completed })
                );
            };
            ...
            const fetch_completed_list = async () => {
                ...
                dispatch(
                    dispatch_list_completed({ todo: completed, completed })
                );
            };
                ...
                for (let i = 0; i < tags_today.length; i++) {
                    if (tags_today.length < 6) {
                        ...
                        if (i < 5) {
                            ...
                            const new_list = tags_today.slice(
                                5,
                                tags_today.length + 1
                            );
                            chart_data_today.values.push(
                                new_list.reduce(
                                    (a, b) => a + b.today.focused / 60,
                                    0
                                )
                            );
                ...
                for (let i = 0; i < tags_week.length; i++) {
                    if (tags_week.length < 6) {
                        ...
                        if (i < 5) {
                            ...
                            const new_list = tags_week.slice(
                                5,
                                tags_week.length + 1
                            );
                            chart_data_week.values.push(
                                new_list.reduce(
                                    (a, b) => a + b.week.focused / 60,
                                    0
                                )
                            );
