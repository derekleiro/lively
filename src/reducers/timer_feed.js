	...
	switch (action.type) {
		...
		case "HANDLE_CHART_DATA_EDIT":
			...
			if (label_index_today !== -1) {
				const split = state.today.labels[label_index_today].split(":");
				state.today.labels[label_index_today] = `${split[0]}: (${readableTime(
					state.today.values[label_index_today] * 60 + action.payload.focustime
				)})`;

				state.today.values[label_index_today] =
					state.today.values[label_index_today] + action.payload.focustime / 60;
			} else {
				const split_two = state.today.labels[
					state.today.values.length - 1
				].split(":");
				state.today.labels[state.today.values.length - 1] = `${
					split_two[0]
				}: (${readableTime(
					state.today.values[state.today.values.length - 1] * 60 +
						action.payload.focustime
				)})`;
			...
			if (label_index !== -1) {
				const split_week = state.week.labels[label_index].split(":");
				state.week.labels[label_index] = `${split_week[0]}: (${readableTime(
					state.week.values[label_index] * 60 + action.payload.focustime
				)})`;
				state.week.values[label_index] =
					state.week.values[label_index] + action.payload.focustime / 60;
			} else {
				const split_week_two = state.week.labels[
					state.week.values.length - 1
				].split(":");
				state.week.labels[state.week.values.length - 1] = `${
					split_week_two[0]
				}: (${readableTime(
					state.week.values[state.week.values.length - 1] * 60 +
						action.payload.focustime
				)})`;
		...
		case "HANDLE_CHART_DATA_NEW_TAG":
			if (state.today) {
				if (state.today.labels.length < 5) {
					...
					state.today.labels.push(new_label);
